...
impl Stack 
{
    ...
    { 
        ...
        match matched_entries.first()
        {
            ...
            None => Err(ApxError::CommandError { error: "Failed to create Stack".into() }.into())
        }
    ...
    {
        let res = run_apx(
            &format!("stacks update --name {} --base {} --packages {} --pkg-manager {}",
                            self.name,
                            self.base, 
                            self.packages.join(" "), 
                            self.package_manager
...
impl Subsystem 
{
    ...
    pub fn create(&mut self) -> Result<()>
    { 
        let mut command = format!("subsystems new --name {} --stack {}",
        self.name,
        self.stack.name);
...
impl PackageManager 
{
    ...
    { 
       ...
       --search '{} --show {} --update {} --upgrade {}",
        ...
        let matched_entries: Vec<&PackageManager> = all.iter().filter(|e| e.name == self.name).collect();
        ...
        match matched_entries.first()
        {
            ...
            None => Err(ApxError::CommandError { error: "Failed to create Package Manager".into() }.into())
        }
